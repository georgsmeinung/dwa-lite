...
# Cargar variables específicas para DWA
DB_PATH = os.getenv("DB_PATH", "")
DWA_DICT_PATH_CSV = os.getenv("DWA_DICT_PATH_CSV", "")
DWA_COL_DICT_PATH_CSV = os.getenv("DWA_COL_DICT_PATH_CSV", "")

# Resolver rutas relativas al script
ruta_db = os.path.join(BASE_DIR, DB_PATH)
ruta_csv = os.path.join(BASE_DIR, DWA_DICT_PATH_CSV)
ruta_col_csv = os.path.join(BASE_DIR, DWA_COL_DICT_PATH_CSV)
...
cursor.execute("DELETE FROM MET_Tables WHERE layer = 'DWA'")
cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'DWA_%'")
dwa_tables = cursor.fetchall()

for (table_name,) in dwa_tables:
    ...
    cursor.execute(
        ...
        VALUES (?, 'DWA', ?, datetime('now'), 'headless', datetime('now'))""",
...
cursor.execute("DELETE FROM MET_Columns WHERE tableName LIKE 'DWA_%'")
