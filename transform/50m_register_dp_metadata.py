...
# Cargar variables específicas para DP
DB_PATH = os.getenv("DB_PATH", "")
DP_DICT_PATH_CSV = os.getenv("DP_DICT_PATH_CSV", "")
DP_COL_DICT_PATH_CSV = os.getenv("DP_COL_DICT_PATH_CSV", "")

# Resolver rutas relativas al script
ruta_db = os.path.join(BASE_DIR, DB_PATH)
ruta_csv = os.path.join(BASE_DIR, DP_DICT_PATH_CSV)
ruta_col_csv = os.path.join(BASE_DIR, DP_COL_DICT_PATH_CSV)
...
cursor.execute("DELETE FROM MET_Tables WHERE layer = 'DP'")
cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'DP_%'")
dp_tables = cursor.fetchall()

for (table_name,) in dp_tables:
    ...
    cursor.execute(
        ...
        VALUES (?, 'DP', ?, datetime('now'), 'headless', datetime('now'))""",
...
cursor.execute("DELETE FROM MET_Columns WHERE tableName LIKE 'DP_%'")
